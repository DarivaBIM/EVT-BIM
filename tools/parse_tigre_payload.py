#!/usr/bin/env python3
"""Parser do payload Tigre (.txt) para tigre_codes.json (schema Slice 2A).
...
# Par com polegada: "20x1/2"" ou "20x1\"" — dn1 mm, dn2 ignorado
RE_PAIR_INCH = re.compile(
    r'(?<![\d.,/])(\d+)\s*[xX]\s*(\d+(?:\.\d+)?(?:/\d+)?)\s*[\'\"]',
)
...
def parse_payload(payload_path: str):
    ...
    with open(payload_path, encoding='utf-8') as f:
        for raw in f:
            ...
            diameter_mm, dn1, dn2 = extract_dims(description)
