...
| Ângulos via matriz robusta (abs(dot) + maior distância pra inline pair) | ✅ aceito (Codex HIGH #5) |
...
1. FILTRO físico (via ConnectorPhysicalFilter):
   - Connector.IsConnected ou Connector.AllRefs.Size > 0 OK
   ...
   - Connector.Radius > 0 (descartar lógicos)
...
6. CLASSIFICAÇÃO:
   ...
   count=2, NOT inline:    BaseKind=Elbow
                           NominalAngleDeg = round(angle[0,1] / 5) * 5
                                          // arredonda pro múltiplo de 5°
...
**Edge cases handled:**
- **Joelho 45_90 instance setada em 45°**: matrix de ângulos retorna 45°, BaseKind=Elbow, NominalAngleDeg=45 → matcher catálogo bate `Joelho 45 REDUX DN50`, não ambíguo.
